...
jQuery(document).ready(function ($){
    ...
    var Expo360obj = function(parameters){
    	...
    	this.$main = $("<div id=\"main\"></div>");
    	this.$view = $("<div id=\"view\"></div>");
    	this.$imagesHolder = $("<div id=\"imagesHolder\"></div>");
        this.$bigContent = $("<div id=\"bigContent\"></div>");
    	this.$panel = $("<div id=\"panel\"></div>");
    	this.$loader = $("<div id=\"loader\"></div>");
    	this.$zoomWindow, this.$zoomWindowBox, this.$zoomClickable;
    	
    	this.$loadingText = $("<div id=\"loadingText\"></div>");
    	...
    	this.sliderFrom, this.sliderTo, this.sliderX;
     	...
     	this.goToDegree = false, this.showedHotspots = false, this.focusedNum = -1, this.smallTweenTime =1;
    ...
    Expo360obj.prototype = {
    	...
    	parseXml: function(xml){
    		...
    		this.ease = 1;//parseFloat($(configuration).find("ease").text(), 10);
    		this.paddingEase =  1;//parseFloat($(configuration).find("padding_ease").text(), 10);
    		this.inertia = 1; //parseFloat($(configuration).find("inertia").text(), 10);
            ...
            this.tooltips_options.fadeTime = 0;// parseInt($(tooltips_xml).find("fadeTime").text(), 10);
    			...
    			if(spots.length > 0){
    				...
    				$(spots).each(function(index){
    					...
    					if(obj.type == "small"){
                            ...
                            contObj.opacity = $(content).find("opacity").text();
		...
		this.$imagesHolder.css({
            ...
            for(var i=0 ; i< this.controls.length; i++){
                if(	this.controls[i] == "left" ||
                    ...
                    var $btn = $('<a id=\"panel-button\" href="#"></a>').appendTo(this.$panel);
                    ...
                    ///////////////////
                    //PLAYBACK SLIDER
                    var $playback_slider = $('<div id=\"playback_slider\"></div>');
                    ...
                    var $p_slider = $('<div id=\"p_slider\"></div>');
    		...
    		//MAKE MENU left and right buttons //////////////
    		//for(var i=0 ; i< this.	.length; i++){
    		//	if(	this.controls[i] == "left" ||
    		//		this.controls[i] == "right"){
             //       var $btnFree = $('<a id=\"free-button\" href="#"></a>').appendTo(this.$imagesHolder);
            //
             //       $btnFree.css({
             //           "position":"relative",
             //           "float":"left",
             ...
             //       });
            //
             //       buttonsClass($btnFree, this.panel_options.ui_folder+this.controls[i]+"_out.png", this.panel_options.ui_folder+this.controls[i]+"_over.png", this.panel_options.buttons_width, this.panel_options.buttons_height, this.panel_options.buttons_tween_time);
            //
             //       preventDragDefault($btnFree);
             //       $btnFree.click(function(){return false;});
    		//
             //       if(this.controls[i] == "left")
             //           $btnFree.bind(mouseDownBind, $.proxy(this.leftControlDown, this));
            //
             //       else if(this.controls[i] == "right")
             //           $btnFree.bind(mouseDownBind, $.proxy(this.rightControlDown, this));
            //
    		//
    		//	}
            //
    	...
    	loadImage: function (num){
			...
			if (isIE) {
    			...
    			var $image = $("<div id=\"image"+num+"\" rel='"+num+"'></div>");
    			$image.css({
    				"position":"absolute",
    				"width":"100%",
				"top":"0"
	    ...
	    $("#viewer>div").first().css("min-height","150px");
		...
		$("#viewer>div>div").first().css("height","initial");
		$("#viewer>div>div").css("position","initial");
		
		$("#viewer>div>div>div").first().css("position","initial");
		$("#viewer>div>div>div").first().css("width","100%");
		$("#viewer>div>div>div").first().css("height","initial");
		
		
		//$("#viewer>div>div>div>div").css("position","initial");
		//$("#viewer>div").first().css("position","initial");
		$("#viewer>div").first().css("height","initial");
		...
		$("#goRight").parent().css("top",$("#viewer").innerHeight()/2-40-120+"px");
		
		$("#goLeft").parent().css("top",$("#viewer").innerHeight()/2-40-120+"px");
    	...
    	onRotateMove: function (e){
    		...
    		if(ismobile){
				...
				$("#dummydiv").remove();
				$("body").append("<div id=\"dummydiv\" style=\"display:none;\">"+randomNumber+"</div>");
				
	    		if(ignored != e.pageX && ignore2 > 2){
	    	    	//this.currentX = e.pageX;
					//console.log("this is : "+e.pageX);
					//console.log(this.currentX );
					
					
					
					this.currentX = this.currentX  + (Math.round((lastObjectPosition-e.pageX)));
					
					
					
					
					//$("#debug").html("1- mobile:"+e.pageX);
				}
	    	    else if(ignore2 <= 2){
	    	    	ignored = e.pageX;
	    	    	ignore2++;
					//console.log("this is : "+ignore2);
					//$("#debug").html("2- mobile:"+e.pageX);
					
	    	    }
				lastObjectPosition = e.pageX;
				
    	   	}
    	   	else{
		
			
			if (Math.abs(lastObjectPosition-e.pageX)<50) {
			    this.currentX = this.currentX +  Math.round((lastObjectPosition-e.pageX)/2) ;
    	...
    	clearSmall: function (){
    	    if(this.$hotspotsSmall != null){
    	       this.$hotspotsSmall.stop().fadeTo(0, 0, function(){
    	           $(this).remove();
    	       });
	...
	//console.log("hello");
            ...
            if(this.$hotspotsSmall != null){
					...
					//console.log(Math.round($("#imagesHolder").innerWidth*0.2/2));
                this.$hotspotsSmall.css({
                    "left": Math.round($("#imagesHolder>div:first-child").innerWidth()*0.25/2),
                    "top": Math.round($("#imagesHolder>div:first-child").innerHeight()*0.05),
					"max-height":   Math.round($("#imagesHolder>div:first-child").innerHeight()*0.9)
    	...
    	getHotspotButton: function (hotspot, rel){
    		...
    		var $btn = $("<a href='#' id=\"hotspotButton"+rel+"\" rel="+rel+"></a>").css({
    			"position":"absolute",
		...
		//var n = str.indexOf("welcome"); 
		    ...
		    if (hotspot.content.indexOf("javascript:")>=0) {
		    
			
    			
			if(ismobile){
			//console.log(ismobile)
			    var putcontent =  hotspot.content.replace("javascript:", ""); 
			    //$btn.attr({"onclick": putcontent, "href": hotspot.content });
			    $btn.bind(clickBind, function(){eval(putcontent); return false;});
			}
			else{
			   var putcontent =  hotspot.content.replace("javascript:", ""); 
			    $btn.attr({"onclick": putcontent, "href": "javascript:void(0)" });
			}
	        ...
	        $btn.attr({"class": "pulse"});
                ...
                $btn.bind(clickBind, function(){
                    ...
                    var $hotspot = $(this);
                    if (ismobile) {
			/*var $small_content = $("<div id=\"hotspot-holder\"></div>").css({
                        "width":"100%",
			            "text-align":"left",
			            "z-index":"214748364",
						"left":"10%",
    	...
    	clearHotspots: function (){
			...
			this.currentHotspots[i].addClass("disabled removing").stop().fadeTo(0, 0, function(){
				$(".pulse").removeClass("pulse2");
    				$(this).remove();
    			});
    		}
    		for(var i = 0; i<this.currentHotspotsTooltips.length ; i++){
                this.currentHotspotsTooltips[i].stop().fadeTo(0, 0, function(){
                    $(this).remove();
                });
            }
	    ...
	    //console.log(this.currentHotspots.length);
    		...
    		for(var i = 0; i<this.currentHotspots.length ; i++){
				...
				var hotLeft = (Math.round(($("#imagesHolder>div:first-child").innerWidth()*hotspot.x/100)));
				var hotTop = (Math.round(($("#imagesHolder>div:first-child").innerHeight()*hotspot.y/100)));
				
				//console.log(hotspot.x+"/"+hotspot.y);
				
				/*
				var hotLeft = (Math.round(($("#RMPContentDiv").innerWidth()*hotspot.x/100)));
				var hotTop = (Math.round(($("#viewer").innerHeight()*hotspot.y/100)));*/
				;
				
				this.currentHotspots[i].css({
    					"left" : Math.round(hotLeft * this.zoomCurrent) +"px",
    					"top" : Math.round(hotTop * this.zoomCurrent) +"px"
    				});
    	...
    	bringHotspotsTimeout: function (){
            ...
            if(this.hotspots[this.currentImage] != null && $("#imagesHolder").innerWidth()>478 ){
		
		/*console.log($("#imagesHolder").innerWidth());*/
		    ...
		    var hotLeft = (Math.round(($("#RMPContentDiv").innerWidth()*hotspot.x/100)))+"px";
		    var hotTop = (Math.round(($("#RMPContentDiv").innerWidth()*hotspot.y/100)))+"px";
		    ...
		    for(var i =0; i<this.hotspots[this.currentImage].length ; i++){
		    var hotspot = this.hotspots[this.currentImage][i];
		    //console.log("HOTSPOT"+ hotspot.type);
	    ...
	    //console.log("hello hotspot");
	    
    	    if(this.hotspots[this.currentImage] != null){
		
                this.hotspotsTimer = setTimeout($.proxy(this.bringHotspotsTimeout, this), 0);
    	    }
    	},
    	getToFront: function (){
			...
			//console.log(this.currentImage+"/"+ this.oldImage);
								
				/*$("#image"+this.currentImage).css("z-index","-1");
				$("#image"+this.oldImage).css("z-index","-2");*/
				
				$("#image"+this.currentImage).css("z-index","0"); 
			 if (ismobile)	$("#image"+this.currentImage).css("display", "block");
				
        
				$("#imagesHolder>div:not([rel="+this.currentImage+"])").css("z-index","-1");
			if (ismobile)	$("#imagesHolder>div:not([rel="+this.currentImage+"])").css("display", "none");
			...
			/*
				...
				//this.images[this.currentImage].show(2);	
    				//this.images[this.currentImage].show(2);
				/*
    				if(this.include_zoom_window)
    	...
    	calculateDegree: function (){
    		...
    		if(!this.dragging && !this.goToDegree && !this.draggingPlaybackSlider){
			    ...
			    degreeEase = 0.7;//((this.goToDegreeNum-this.degrees)/this.ease);
		...
		/*	
    		...
    		//
		if (Hover360) setTimeout($.proxy(this.updateImage, this), refreshRate);
    	},
    	///////////////////////////////////////////////
    ...
    Expo360 = function( parameters ){
    	...
    	if (fileExists(parameters.xml)) {
            ...
            this.getCurrentHotspots = function () {


            }

            this.getCurrenthotspot = function () {
            ...
            this.closeLastHotspot = function () {
                expo.clearSmall();
            }

            /*
            this.updateThisHotspots = function(){
                expo.bringHotspots();
            }*/
            ...
            expo.rightControlDown();
            }

            this.goLeft = function(){
            expo.leftControlDown();
            }
            */
            this.stopSpinning = function () {

                expo.leftRightControlUp();
            }
